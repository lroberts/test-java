...
<div class="item-list views-accordion <?php print $views_accordion_id ?>">
  ...
  <div id="<?php print $views_accordion_id ?>">
    <?php foreach ($rows as $id => $row): ?>
      <div id="<?php print 'A'. $id; ?>" class="<?php print $classes[$id] ?>"><?php print $row ?></div>
    <?php endforeach; ?>
  </div>
</div>

<?php if ($views_accordion_id == 'views-accordion-fee_tables-page_1'): ?>
  <script type="text/javascript">$(document).ready(function(){if(location.hash != '' && $('div' + location.hash).size()){$('#views-accordion-fee_tables-page_1 '+location.hash+' .accordion-content').show();$('#views-accordion-fee_tables-page_1 '+location.hash+' .accordion-header').addClass('accordion-header-active');}});</script>
<?php endif; ?>