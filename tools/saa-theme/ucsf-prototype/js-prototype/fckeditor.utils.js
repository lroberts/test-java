...
Drupal.behaviors.fckeditor = function(context) {
  ...
  jQuery('textarea.fckeditor:not(.fckeditor-processed)', context).each(function() {
    var textarea = jQuery(this).addClass('fckeditor-processed');
    ...
    if (fckInstances[taid]) {
      ...
      if (editorInstance.defaultState == 1) {
        if (textarea.attr('class').indexOf("filterxss1") != -1 || textarea.attr('class').indexOf("filterxss2") != -1) {
          jQuery.post(Drupal.settings.basePath + 'index.php?q=fckeditor/xss', {
            text: jQuery('#' + taid).val(),
            'filters[]': Drupal.settings.fckeditor_filters[fckInstances[taid].DrupalId]
            },
            function(text) {
              textarea.val(text);
              jQuery('#img_assist-link-' + taid).hide();
              jQuery(".img_assist-button").hide();
...
{
  var swtch = jQuery('#switch_'+textareaID);
  ...
  var editorFrame = jQuery('#'+textareaID+'___Frame');
  ...
  if (textArea.is(':hidden')) {
    ...
    editorFrame.hide();
    jQuery('#img_assist-link-' + textareaID).show();
    jQuery(".img_assist-button").show();
    jQuery(textArea).parent().children(".grippie").show();
  } else {
    ...
    editorFrame.show();
    jQuery('#img_assist-link-' + textareaID).hide();
    jQuery(".img_assist-button").hide();
  }
}
...
function FCKeditor_OnComplete(editorInstance) {
  ...
  jQuery('#switch_' + editorInstance.Name).show();
  ...
  jQuery(editorInstance.LinkedField).parent().children('.grippie').each(function() {
  ...
  editorInstance.LinkedField = jQuery('<textarea></textarea>');
...
function FCKeditor_TeaserInfo(taid) {
  ...
  if (fckTeaser.lookup[taid]) {
    var obj = {
      textarea : jQuery('#'+fckTeaser.lookup[taid]),
      checkbox : jQuery('#'+Drupal.settings.teaserCheckbox[fckTeaser.lookup[taid]])
    };

    obj.textareaContainer = obj.textarea.parent();
    obj.checkboxContainer = obj.checkbox.parent();

    obj.button = jQuery('input.teaser-button', obj.checkbox.parents('div.teaser-checkbox').get(0));
