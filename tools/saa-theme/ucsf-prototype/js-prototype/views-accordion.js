...
Drupal.behaviors.views_accordion = function(context) {
  ...
  if(Drupal.settings.views_accordion){
    jQuery.each(Drupal.settings.views_accordion, function(id) {
      ...
      var $viewcontent = jQuery('#' + displaySelector);
       ...
       */
      var hasRan = jQuery(idSelector + ' ' + contentSelector).length;
      //console.log(hasRan ? idSelector + ' already ran' : idSelector + ' had not ran already');  // for debugging

      if (!hasRan) {
        var $triggers = usegroupheader ? jQuery(headerSelector, $viewcontent) : jQuery(idSelector + ' ' + headerSelector);
        $triggers.addClass('accordion-header');

        $triggers.parent().each(function(){
          jQuery(this).children().slice(1).wrapAll('<div class="' + contentClass + '">') // we wrap all but the header in a div so we can target the content later
        }).parent().addClass('accordion-active');

        var $content =  usegroupheader ? jQuery(contentSelector, $viewcontent) : jQuery(idSelector + ' ' + contentSelector);
        $content.hide();
        ...
        if (!cycleOn && togglelinks) {
            ...
            jQuery('a.close-all-accordion', $toggleContainer).click(function(){
                $content.hide();
                $triggers.removeClass(activeClass);
                return false;
              });
            jQuery('a.open-all-accordion', $toggleContainer).click(function(){
                $content.show();
        ...
        $triggers.click(function(ev) {
          if (ev.detail === 1 || !ev.detail) { // so we prevent double clicking madness (for not so savy web users) !ev.detail is for when its triggered by code
            ...
            var $ourContent = jQuery(this).next();
