...
//import junit.framework.RuntimeException;
//import org.junit.*;
...
{
    /**
     *  We subclassed ActionServlet to to allow the location of web.xml to
     *  be configurable. The source for initServlet() is taken from struts 1.2.9
     */
    protected boolean useCustomizedActionServlet = true;
    protected InputStream altWebXml = null;
    ...
    public MockStrutsTestCase(String s) {
        super();
    }

    class WebXmlConfigurableActionServlet extends ActionServlet
    {
	InputStream input;
	public WebXmlConfigurableActionServlet(InputStream in){
	    super();
	    this.input = in;
	}
    ...
    private void init() {
        if (!isInitialized)
            throw new RuntimeException("You are overriding the setUp() method without calling super.setUp().  You must call the superclass setUp() method in your TestCase subclass to ensure proper initialization.");
    }
    ...
    public void setUp() throws Exception {
	    ...
	    if(!useCustomizedActionServlet){
		actionServlet = new ActionServlet();
	    }else{
		actionServlet = new WebXmlConfigurableActionServlet(altWebXml);
	    }
    ...
    public void reset(boolean transferSession){
	HttpSession curr = request.getSession();
	request = new servletunit.HttpServletRequestSimulator(config.getServletContext());
	if(transferSession)
	    request.setSession(curr);
        response = new servletunit.HttpServletResponseSimulator();
    ...
    public void setActionServlet(ActionServlet servlet) {
        ...
        init();
        if (servlet == null)
            throw new RuntimeException("Cannot set ActionServlet to null");
     ...
     *
     * @exception RuntimeException if there are any execution
    ...
    public void setServletConfigFile(String pathname) {
        ...
        try {
            ...
            if(input==null)
                throw new RuntimeException("Invalid pathname: " + pathname);
            digester.parse(input);
            input.close();
        } catch (Exception e) {
	    logger.error("Received an exception while loading web.xml - " + e.getClass() + " : " + e.getMessage());
            throw new RuntimeException("Received an exception while loading web.xml - " + e.getClass() + " : " + e.getMessage());
        }
     ...
     *
     * @exception RuntimeException if the ActionServlet controller
    ...
    //throws RuntimeException 
    {
        ...
        if (actualForward == null) {
            ...
            throw new RuntimeException("Was expecting '" + forwardPath + "' but it appears the Action has tried to return an ActionForward that is not mapped correctly.");
        }
        if (logger.isDebugEnabled()) {
            logger.debug("expected forward = '" + forwardPath + "' - actual forward = '" + actualForward + "'");
        }
        if (!(actualForward.equals(forwardPath)))
            throw new RuntimeException("was expecting '" + forwardPath + "' but received '" + actualForward + "'");
        if (logger.isDebugEnabled())
            logger.debug("Exiting");
    }
    ...
    public void verifySessionActionMessages(String[] messageNames) {
        if (logger.isDebugEnabled())
            logger.debug("Entering - messageNames = " + messageNames);
        init();

	// NEW: 19jan2012 we check both the request and session
        Common.verifyActionMessages(request.getSession(),messageNames,Globals.MESSAGE_KEY,"action");
        if (logger.isDebugEnabled())
            logger.debug("Exiting");
    }
