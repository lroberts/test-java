...
public class Common {
    ...
    protected static void verifyNoActionMessages(HttpServletRequest request, String key, String messageLabel) {
        ...
        if (messages != null) {
            ...
            if (iterator.hasNext()) {
                ...
                throw new RuntimeException("was expecting no " + messageLabel + " messages, but received: " + messageText.toString());
            }
        }
        if (logger.isTraceEnabled())
    ...
    protected static void verifyActionMessages(HttpServletRequest request, String[] messageNames, String key, String messageLabel) {
        ...
        if (messages == null) {
            throw new RuntimeException("was expecting some " + messageLabel + " messages, but received none.");
        }
        /* check length of messages as optimization */
        else if (messages.size() != messageNames.length) {
            throw new RuntimeException("was expecting " + messageNames.length + " " + messageLabel + " message(s), but received " + messages.size() + " " + messageLabel + " message(s)");
        }
        else  {
            ...
            for ( int i = 0; i < messageNames.length;i++) {
                if ( !messageNames[i].equals(messageKeys[i])) {
                    ...
                    throw new RuntimeException("received " + messageLabel + " messages: (" + mks + ") but expected (" + mns + ")");
                }
            }
        }
        if (logger.isTraceEnabled())
    ...
    protected static void verifyActionMessages(HttpSession session, String[] messageNames, String key, String messageLabel) {
        if (logger.isTraceEnabled())
            logger.trace("Entering - session = " + session + ", messageNames = " + messageNames + ", key = " + key + ", messageLabel = " + messageLabel);
     ...
     *
     * @throws RuntimeException if the expected and actual tiles definitions do not match.
     */
    protected static void verifyTilesForward(String actionPath, String forwardName, String expectedDefinition, boolean isInputPath, HttpServletRequest request, ServletContext context, ServletConfig config) {
        ...
        if ((forwardName == null) && (isInputPath)) {
            ...
            if (forwardName == null)
                throw new RuntimeException("Trying to validate against an input mapping, but none is defined for this Action.");
        ...
        if (!isInputPath) {
            ...
            if (expectedForward == null)
                throw new RuntimeException("Cannot find forward '" + forwardName + "'  - it is possible that it is not mapped correctly.");
        ...
        if (definitionName == null)
            throw new RuntimeException("Could not find tiles definition mapped to forward '" + forwardName + "'");
        if (!definitionName.equals(expectedDefinition))
            throw new RuntimeException("Was expecting tiles definition '" + expectedDefinition + "' but received '" + definitionName + "'");
        if (logger.isTraceEnabled())
            logger.trace("Exiting");
    }
     ...
     *
     * @throws RuntimeException if expected and actual paths do not match.
     */
    protected static void verifyForwardPath(String actionPath, String forwardName, String actualForwardPath, boolean isInputPath, HttpServletRequest request, ServletContext context, ServletConfig config) {
        ...
        if (!isInputPath) {
            ...
            if (forwardName == null) {
                if (actualForwardPath == null)
                    ...
                    throw new RuntimeException("Expected a null forward from action, but received '" + actualForwardPath + "'");
            }
        ...
        if (actualForwardPath == null) {
            ...
            throw new RuntimeException("Was expecting '" + forwardName + "' but it appears the Action has tried to return an ActionForward that is not mapped correctly.");
        }
        if (logger.isDebugEnabled()) {
        ...
        if (!forwardName.equals(stripJSessionID(actualForwardPath)))
            throw new RuntimeException("was expecting '" + forwardName + "' but received '" + actualForwardPath + "'");
        if (logger.isTraceEnabled())
            logger.trace("Exiting");
    }
